...
No escopo atual do template, o suficiente para a demo é manter o `trace_id`, a latência e o evento do routing registrados no log estruturado. Isso já permite explicar o fluxo sem depender de uma ferramenta externa.

Como o projeto agora pode rodar com Groq para chat e embeddings locais, vale registrar também se a chamada foi respondida pelo cache, pelo modelo barato ou pelo modelo premium. Esses eventos são os que melhor ajudam a demonstrar redução de custo no vídeo.
